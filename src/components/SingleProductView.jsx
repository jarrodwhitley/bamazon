  ...
  return (
    <>
      {selectedProduct && Object.keys(selectedProduct).length > 0 && (
        <div
          className={
            "single-product-view animate__animated fixed top-0 left-0 right-0 h-full max-h-screen flex items-center justify-center bg-white bg-opacity-80 " +
        ...
        >
          <div
            className={
              "single-product-view__modal animate__animated w-full md:w-3/4 lg:w-[1000px] h-full lg:min-h-[650px] md:max-h-[60vh] bg-white grid grid-rows-[1fr_2fr] lg:grid-rows-1 grid-cols-1 lg:grid-cols-2 items-start gap-4 md:p-6 shadow-2xl overflow-auto relative " +
              (!closingModal
          ...
          >
            <div
              className="single-product-view__close fixed top-0 right-0 px-4 py-2 text-2xl cursor-pointer"
            ...
            <div className="single-product-view__images row-start-1 col-start-1 grid grid-cols-1 grid-rows-1 w-full h-[30vh] lg:h-full">
                ...
                <div className="single-product-view__mobile-image-gallery overflow-hidden w-screen">
                  <div className="single-product-view__mobile-image-gallery-snap-container h-full lg:h-[400px] snap-x snap-mandatory flex overflow-auto">
                    {selectedProduct.images.length > 0 &&
                      selectedProduct.images.map((image, index) => (
                        <figure
                          key={index}
                          className="snap-center row-start-1 w-screen h-full"
                        >
                          <img
                            src={image}
                  ...
                  <div className="single-product-view__scroll-indicator flex justify-center w-full absolute">
                    {selectedProduct.images.map((image, index) => (
                      <div
                        key={index}
                        className={`single-product-view__scroll-indicator-dot w-2 h-2 mx-1 rounded-full ${index === currentImageIndex ? "bg-gray-500" : "bg-gray-300"}`}
                      />
                    ))}
                  </div>
                </div>
              ...
              {showBam && (
                <img
                  src={Boombam}
                  alt="Bamazon logo"
                  className="animate__animated animate__bounceIn animate__faster single-product-view__bam absolute top-0 left-0 w-2/3 drop-shadow-2xl z-10"
                />
              )}
            </div>
            <div className="single-product-view__details row-start-2 lg:row-start-1 col-start-1 lg:col-start-2 w-full p-4 md:p-0 ">
              <div className="single-product-view__section">
                ...
                {selectedProduct.stock < 10 && (
                  <div className="single-product-view__low-stock text-red-500 text-xs font-semibold">
                    Low Stock - only {selectedProduct.stock} remaining
                  </div>
                )}
              </div>
              <div className="single-product-view__section grid grid-rows-1 grid-cols-2 mt-4 pt-2 w-full place-self-end">
                <div
                  className="single-product-view__add-to-cart flex items-center justify-center bg-blue-500 text-white font-semibold px-4 py-2 rounded self-start cursor-pointer"
                ...
                {cart.items.find(
                  ...
                  <div className="single-product-view__added-message flex items-center justify-center">
                    <p
                      className={
                        "single-product-view__in-cart text-green-500 font-semibold animate__animated animate__faster " +
                        (showAddedMessage
                          ? "animate__fadeInUp"
              ...
              <div className="single-product-view__section mt-4 pt-2 border-t-2">
                ...
                <p className="single-product-view__description mt-2">
              ...
              <div className="single-product-view__section mt-4 pt-2 border-t-2">
                ...
                <ul className="list-disc pl-6 mt-2">
                  <li className="single-product-view__list-item">
